...
export default function ProspectTracker() {
  ...
  const move = useCallback((id, dir) => {
    setPeople(prev => {
      // Order people the way they currently appear (by myRank, then existing order)
      ...
      const j = i + dir;
      if (j < 0 || j >= ranked.length) return prev;
      [ranked[i], ranked[j]] = [ranked[j], ranked[i]];
      // Renumber everyone 1,2,3… by new order
      const rankMap = {};
  ...
  return (
    <div style={S.screen}>
      <div style={S.header}>
        <div style={S.title}>Prospects</div>
      ...
      <div style={S.list}>
        ...
        {visible.map((p) => {
          ...
          const overallIdx = people.findIndex(x => x.id === p.id);
          ...
          return (
            <div key={p.id} style={S.row} onClick={() => setOpenId(p.id)}>
              ...
              <div style={S.rank}>{overallIdx + 1}</div>
              {p.photos[0]
              ...
              <div style={S.rowArrows} onClick={e => e.stopPropagation()}>
                <button style={S.arrowBtn} onClick={() => move(p.id, -1)}>▲</button>
                <button style={S.arrowBtn} onClick={() => move(p.id, 1)}>▼</button>
              </div>
            </div>
          );
